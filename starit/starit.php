<?php
/**
 * Plugin element to render starit-up/down widget
 *
 * @package     Joomla.Plugin
 * @subpackage  Fabrik.element.starit
 ...
 * @since       3.0
 */
class PlgFabrik_Elementstarit extends PlgFabrik_Element
{
	/**
	...
	public function renderListData($data, stdClass &$thisRow, $opts = array())
	{
		...
		$imagePath = COM_FABRIK_LIVESITE . 'plugins/fabrik_element/starit/images/';
		...
		for ($i = 0; $i < count($data); $i++)
		{
			...
			$mystarit                     = $this->getMystarit($listId, $formId, $rowId);
			$count                       = $this->_renderListData($data[$i], $thisRow);
			$count                       = FabrikWorker::JSONtoData($count, true);
			$layout                      = $this->getLayout('list');
			$layoutData                  = new stdClass;
			$layoutData->commentdata     = 'data-fabrik-starit-rowid="' . $rowId . '"';
			$layoutData->formId          = $formId;
			$layoutData->upActiveClass   = $mystarit === 'up' ? ' btn-success' : '';;
			$layoutData->downActiveClass = $mystarit === 'down' ? ' btn-danger' : '';;
	...
	private function _renderListData($data, $thisRow)
	{
		...
		return $this->getstaritCount($data, $listId, $formId, $rowId);
	}
	 ...
	 *
	 * @return count starit-up, count starit-down
	 */
	protected function getstaritCount($data, $listId, $formId, $rowId)
	{
		...
		$db
			->setQuery(
				"SELECT COUNT(starit) FROM #__{package}_starit WHERE listid = " . (int) $listId . " AND formid = " . (int) $formId . " AND row_id = "
					. $db->q($rowId) . " AND element_id = " . (int) $elementId . $sql . " AND starit = 'up'");
		$resup = $db->loadResult();
		$db
			->setQuery(
				"SELECT COUNT(starit) FROM #__{package}_starit WHERE listid = " . (int) $listId . " AND formid = " . (int) $formId . " AND row_id = "
					. $db->q($rowId) . " AND element_id = " . (int) $elementId . $sql . " AND starit = 'down'");
	...
	/**
	 * Get All lists starit in 2 queries.
	 *
	 * @return  array
	 */
	public function getListstaritCount()
	{
		...
		foreach (array('up', 'down') as $dir)
		{
			...
			$query->select('COUNT(starit) AS up, row_id')->from('#__{package}_starit')
			->where('listid = ' . (int) $listId . ' AND formid = ' . (int) $formId . ' AND starit = ' . $db->q($dir));
	...
	public function render($data, $repeatCounter = 0)
	{
		...
		$imageFileUp = 'starit_up_out.gif';
		$imageFileDown = 'starit_down_out.gif';

		if ($mystarit == 'up')
		{
			$imageFileUp = 'starit_up_in.gif';
			$imageFileDown = 'starit_down_out.gif';
		}
		elseif ($mystarit == 'down')
		{
			$imageFileUp = 'starit_up_out.gif';
			$imageFileDown = 'starit_down_in.gif';
		}
	 ...
	 * @param   string  $rowId  Row id
	 *
	 * @return string  starit value
	 */
	protected function getMystarit($listId, $formId, $rowId)
	{
		$db = FabrikWorker::getDbo();
		$elementId = $this->getElement()->id;
		...
		$query->select('starit')->from('#__{package}_starit')
	...
	/**
	 * Called via widget ajax, stores the selected starit
	 * stores the diff (starit-up minus starit-down)
	 *
	...
	public function onAjax_rate()
	{
		...
		$starit = $input->get('starit');
		$add = $input->get('add', 'true');

		if ($add === 'true')
		{
			$this->dostarit($listId, $formId, $rowId, $starit);
		}
		else
		{
			$this->deletestarit($listId, $formId, $rowId, $starit);
		}

		echo $this->getstaritCount('', $listId, $formId, $rowId);
	}
	...
	{
		$cookieName = 'starit-table_' . $listId . '_row_' . $rowId . '_ip_' . FabrikString::filteredIp();
	 ...
	 * @param   int     $formId  Form id
	 * @param   string  $rowId  Row id
	 * @param   string  $starit   starit value
	 *
	 * @return  void
	 */
	private function deletestarit($listId, $formId, $rowId, $starit)
	{
		$userId = $this->getUserId($listId, $rowId);
		$db = FabrikWorker::getDbo();
		$query = $db->getQuery(true);
		$query->delete('#__{package}_starit')->where('user_id = ' . $db->q($userId))
		->where('listid = ' . $listId . ' AND row_id = ' . $rowId . ' AND starit = ' . $db->q($starit));
		$db->setQuery($query);
		$db->execute();
		$elementId = $this->getElement()->id;
	 ...
	 * @return  void
	 */
	private function dostarit($listId, $formId, $rowId, $starit)
	{
		if (!$this->canUse())
		{
			return;
		...
		$db->setQuery(
			"INSERT INTO #__{package}_starit
				(user_id, listid, formid, row_id, starit, date_created, element_id, special)
				values (
					" . $db->q($userId) . ",
					" . $db->q($listId) . ",
					" . $db->q($formId) . ",
			...
			$db
				->setQuery(
					"UPDATE " . $db->quoteName($this->getlistModel()->getTable()->db_table_name) . "
	                    SET " . $db->quoteName($this->getElement()->name) . " = ((SELECT COUNT(starit) FROM #__{package}_starit WHERE listid = " . (int) $listId
						. " AND formid = " . (int) $formId . " AND row_id = " . $db->q($rowId) . " AND element_id = " . (int) $elementId
						. " AND starit = 'up') - (SELECT COUNT(starit) FROM #__{package}_starit WHERE listid = " . (int) $listId . " AND formid = "
	...
	public function elementJavascript($repeatCounter)
	{
		...
		$this->lang->load('plg_fabrik_element_starit', JPATH_BASE . '/plugns/fabrik_element/starit');
		$opts = new stdClass;
		$opts->canUse = $this->canUse();
		$opts->noAccessMsg = trim(FText::_($params->get('starit_no_access_msg', FText::_('PLG_ELEMENT_starit_NO_ACCESS_MSG_DEFAULT'))));
		$opts->row_id = $rowId;
		$opts->mystarit = $this->getMystarit($listId, $formId, $rowId);
	...
	public function elementListJavascript()
	{
		...
		foreach ($groupKeys as $gKey)
		{
			foreach ($data[$gKey] as $rowKey)
			{
				...
				$listMystarit[$rowKey->__pk_val] = $this->getMystarit($list->id, $formId, $rowKey->__pk_val);
			}
		}
		...
		$opts = new stdClass;
		$opts->canUse = $this->canUse();
		$opts->noAccessMsg = FText::_($params->get('starit_no_access_msg', FText::_('PLG_ELEMENT_starit_NO_ACCESS_MSG_DEFAULT')));
		$opts->listid = $list->id;
		$opts->formid = $this->getFormModel()->getId();
		$opts->imagepath = COM_FABRIK_LIVESITE . 'plugins/fabrik_element/starit/images/';
		$opts->elid = $this->getElement()->id;
		$opts->mystarit = $listMystarit;
		$opts->userid = $userId;
		$opts->renderContext = $this->getListModel()->getRenderContext();
		$opts = json_encode($opts);

		return "new FbstaritList('$id', $opts);\n";
	}
	...
	{
		$db = FabrikWorker::getDbo();
		$query = "CREATE TABLE IF NOT EXISTS  `#__{package}_starit` (
	...
	`special` VARCHAR(30),
	 PRIMARY KEY ( `user_id` , `listid` , `formid` , `row_id`, `element_id`, `special` )
		...
		$cols = $db->getTableColumns('#__{package}_starit');

		if (array_key_exists('tableid', $cols))
		{
			$db->setQuery('ALTER TABLE #__{package}_starit CHANGE ' . $db->qn('tableid') . ' ' . $db->qn('listid') . ' INT(6)');
			$db->execute();
		}

		if (!array_key_exists('special', $cols))
		{
			$db->setQuery('ALTER TABLE #__{package}_starit ADD COLUMN ' . $db->qn('special') . ' VARCHAR(30)');
			$db->execute();

			$db->setQuery('ALTER TABLE #__{package}_starit DROP PRIMARY KEY');
			$db->execute();

			$db->setQuery('ALTER TABLE #__{package}_starit ADD PRIMARY KEY (`user_id`, `listid`, `formid`, `row_id`, `element_id`, `special`)');
			$db->execute();
		}
	}
}
